...
<head>
  ...
  <style>
    ...
    *{
    ...
    .hamb-icon span{
      height:2px;background:var(--brown);border-radius:999px;
    }
    .sidebar .nav-link{
...
const innerTgl  = document.getElementById('toggleSidebarInside');
...
innerTgl?.addEventListener('click', closeSide);
backdrop?.addEventListener('click', closeSide);

/* nav link close in mobile */
...
function openEdit(id,row){
  ...
  document.getElementById('category_label').textContent =
    cat === 'food' ? 'Food' : (cat === 'pastry' ? 'Pastry' : 'Drink');
  // set stok
