...
<head>
  ...
  <style>
    :root{
      ...
      --ink:#111827;
      --muted:#6b7280;
      --unread:#FFEBA6; /* latar kuning lembut utk unread */
    }
    *{
    ...
    .notif-unread{
      ...
      border-color:rgba(255,213,79,.30);
    }
    .notif-msg{ font-size:.95rem; line-height:1.6; }
    .notif-time{ font-size:.8rem; color:var(--muted); margin-top:4px; }
    .empty-box{
      background:#fff; border-radius:18px; border:1px dashed rgba(0,0,0,.04);
...
<body>
  ...
  <header class="topbar">
    <div class="inner">
      <a href="<?= htmlspecialchars($BASE) ?>/public/admin/index.php" class="back-link">
  ...
  <script>
    ...
    // setelah halaman tampil, tandai semua dibaca untuk role=admin
    fetch(`${BASE}/backend/api/notifications.php?action=mark_all_read&role=admin`, {
